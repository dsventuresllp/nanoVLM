...
from datasets import load_dataset, concatenate_datasets, get_dataset_config_names, Dataset, DatasetDict
...
def get_dataloaders(train_cfg, vlm_cfg):
    ...
    for dataset_name in dataset_names_to_load:
        try:
            raws = load_dataset(train_cfg.train_dataset_path, dataset_name)
            #handle both DatasetDict and Single Dataset
            if isinstance(raws, DatasetDict): 
                ds = raws['train']
            elif isinstance(raws, Dataset): 
                ds = raws
            else: 
                raise ValueError(f"Unexpected return type: {type(raws)}")
            # Sanity check
            _ = ds[0]
